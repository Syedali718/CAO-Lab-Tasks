...
.globl main01
main01:
la $a0,bubble # print bubble sort
li $v0,4
syscall
la $a0,msg0 # print enter array length
li $v0,4
syscall
li $v0,5 # input integer or array length
syscall
ble $v0,0,invalid_size # prints error message length entered smaller then 0
subi $s0,$v0,1 #$v0 main sy ek minus kren gy aur S0 main save krwa dain gy
#add $s0,$zero,$s0
#addi $t0,$zero,0#########
li $t0,0 # t0 main 0 dal dea hai
in:

la $a0,msg1# print label enter input
li $v0,4
syscall
...
#add $t1,$t0,$zero
#li $t1,0#############
sll $t1,$t0,2 # phr yahan pr hum ny shift left logic kea hai aur 2 steps k sath left move kea hai
#add $t3,$v0,$zero    #     jo bhi input lea tha hum ny v0 main tha usy hum ny t3 main move krwa dea hai is liye zero sy add krwaya hai
move $t3 , $v0
sw $t3,Table ( $t1 )  # sw specified place py content ko store kry ga 
		      #is main kia ho raha hai k t1 ki value t3 main table k variable k sath save ho rahi hai
		      # # t0 main 0 tha lif shift pr t1 main 000 a gea ab yahan hum ny table(000) t3 main save krwaya hai
		      # mtlb yeh hai k jo input element lea hai usy hum usy main table(000) py save krwaon ga e.g table(000)=5
addi $t0,$t0,1  # t0 0 tha pehly ab 1 ho gea hai
slt $t1,$s0,$t0 # agr s0 ki value t0 sy km hui to t1 main 1 a jye ga wrna 0 set ho jye ga initially pehly round main yeh ho raha hai k 
# s0 = 4 so yeh 0 value set kry ga t1 = 0
beq $t1,$zero,in # yahan check kry ga k t1 equal to 0 hai ya nahi agr t1 = 0 jump to in
...
loop1:
addi $t0,$t0,1 #i++
bgt $t0,$a1,endloop1 #if t0 < a1 break; branch greater hogi jesy gi length sy end loop py jump kry ga
